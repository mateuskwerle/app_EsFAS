# backend/maintenance/cleanup_user_by_email_or_idfunc.py
import os, sys, re
...
def main():
    ...
    while i < len(args):
        ...
        if a == "--apply": apply = True; i+=1; continue
        i+=1
    ...
    if not email_n and not idfunc_n:
        print("Uso: python -m backend.maintenance.cleanup_user_by_email_or_idfunc --email EMAIL --idfunc 123456 [--apply]")
        sys.exit(1)
    ...
    with engine.begin() as conn:
        try:
            ...
            if not rows:
                print("Nenhum usuário correspondente encontrado para limpeza.")
                return
            ...
            ids = []
            for r in rows:
                ids.append(r['id'] if 'id' in cols else None)
                preview = ", ".join([f"{c}={r[c]!r}" for c in cols])
                print(" - " + preview)

            if not apply:
                print("\n[Dry-run] Não foi feita nenhuma exclusão. Rode novamente com --apply para remover os registros listados acima.")
                return

            if 'id' in cols:
                del_stmt = delete(users).where(users.c.id.in_([i for i in ids if i is not None]))
            else:
                del_stmt = delete(users).where(or_(*conds))
            affected = conn.execute(del_stmt).rowcount or 0
            print(f"\nRemovidos {affected} registro(s) da tabela 'users'.")
