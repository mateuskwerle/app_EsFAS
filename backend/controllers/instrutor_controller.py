...
# --- ROTA COMPLETAR CADASTRO MODIFICADA ---
@instrutor_bp.route('/completar', methods=['GET', 'POST'])
@login_required
def completar_cadastro():
    # Se o utilizador já tiver um perfil, redireciona para o dashboard
    if current_user.instrutor_profile:
        flash("O seu perfil de instrutor já está completo.", "info")
        return redirect(url_for('main.dashboard'))

    form = EditarInstrutorForm()
    if form.validate_on_submit():
        # Utiliza o novo método do serviço para CRIAR o perfil
        ok, msg = InstrutorService.create_profile_for_user(current_user.id, request.form.to_dict(flat=True))
        flash(msg, 'success' if ok else 'danger')
        if ok:
            return redirect(url_for('main.dashboard'))
